...
export class InvoiceService {
  ...
  async generateBill(data): Promise<any> {
    const amountInLetters = await this.utilService.getAmountInLetters(
      Number.parseInt(data.amount.replace('.', '')),
    );
    const pdfBuffer = await new Promise((resolve) => {
      let writerStream;
      const doc = new PDFDocument(
        this.utilService.getDocSetting(data.templateName),
      );
      ...
      doc
        ...
        .font('Courier-Bold')
        .fontSize(14)
        .text(`#${data.documentId}`, { align: 'right' });
      ...
      doc
        .font('Courier')
        .fontSize(14)
        .text(`${data.client.nit} `, { align: 'center' })
        .font('Courier')
        .fontSize(16);
      //=> TEXT AUXILIAR
      doc
      ...
      //=> AMOUNT IN LETTERS
      doc.font('Courier').fontSize(20).text(`${amountInLetters} PESOS`, {
        align: 'center',
      });
      //=> TEXT AUX
      ...
      doc
        ...
        .font('Courier')
        .fontSize(14)
        .text(`${data.creditor.nit} `, { align: 'center' })
        .font('Courier')
        .fontSize(16);
      //=> TEXT AUXILIAR
      doc
      ...
      for (let i = range.start; i < range.start + range.count; i++) {
        ...
        doc
          ...
          .fontSize(10)
          .text(`Firma: ${data.signature}`, 20, 10, { align: 'right' });
      }
      ...
      const pathData = this.utilService.billPathFile(data.documentId);
      doc.pipe((writerStream = fs.createWriteStream(pathData.path)));
      doc.end();
